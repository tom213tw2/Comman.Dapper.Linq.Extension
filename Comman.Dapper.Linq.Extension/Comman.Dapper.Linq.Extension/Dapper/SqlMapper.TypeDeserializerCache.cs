...
namespace Comman.Dapper.Linq.Extension.Dapper
{
    public static partial class SqlMapper
    {
        private class TypeDeserializerCache
        {
            ...
            private readonly Dictionary<DeserializerKey, Func<IDataReader, object>> readers =
                new Dictionary<DeserializerKey, Func<IDataReader, object>>();
            ...
            internal static Func<IDataReader, object> GetReader(Type type, IDataReader reader, int startBound,
                int length, bool returnNullIfFirstMissing)
            {
                ...
                if (found == null)
                    lock (byType)
                    {
                        ...
                        if (found == null) byType[type] = found = new TypeDeserializerCache(type);
                    }

                return found.GetReader(reader, startBound, length, returnNullIfFirstMissing);
            }

            private Func<IDataReader, object> GetReader(IDataReader reader, int startBound, int length,
            ...
            private struct DeserializerKey : IEquatable<DeserializerKey>
            {
                ...
                public DeserializerKey(int hashCode, int startBound, int length, bool returnNullIfFirstMissing,
                    IDataReader reader, bool copyDown)
                {
                ...
                public bool Equals(DeserializerKey other)
                {
                    ...
                    for (var i = 0; i < length; i++)
                        if ((names?[i] ?? reader?.GetName(startBound + i)) !=
                            (other.names?[i] ?? other.reader?.GetName(startBound + i))
                            ||
                            (types?[i] ?? reader?.GetFieldType(startBound + i)) !=
                            (other.types?[i] ?? other.reader?.GetFieldType(startBound + i))
                           )
